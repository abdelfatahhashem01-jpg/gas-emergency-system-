  ...
  return (
    <style>{`
      * { box-sizing: border-box; }
      ...
      @keyframes mgBannerFlash {
        0%, 100% { background: #D93025; }
        50% { background: #B0271B; }
      }
      .mg-root {
        direction: rtl;
...
function IncidentForm({ currentUser, regions, onSubmitted }) {
  ...
  async function doSubmit() {
    ...
    if (form.address.trim() && !form.location) {
      ...
      geocodeAddress(form.address.trim(), regionName).then(async (coords) => {
        if (!coords) return;
        const latest = await loadJSON("gas_reports", []);
        const updated = latest.map((x) => (x.id === report.id ? { ...x, geocoded: coords } : x));
        ...
        <ReportDetailModal
          ...
          onSave={async (updated) => {
            ...
            if (updated.address?.trim() && !updated.location && updated.address.trim() !== original?.address?.trim()) {
              ...
              geocodeAddress(updated.address.trim(), regionName).then(async (coords) => {
                if (!coords) return;
                const latest = await loadJSON("gas_reports", []);
                const finalNext = latest.map((x) => (x.id === updated.id ? { ...x, geocoded: coords } : x));
...
function Header({ currentUser, onLogout, onChangeCode, onRefresh, tab, setTab, appName }) {
  const tabsByRole = {
    super_admin: ["report", "list", "opswall", "stats", "regions", "users", "settings", "alerts"],
    manager: ["report", "list", "opswall", "stats", "regions", "alerts"],
    response: ["report", "list"],
    observer: ["list", "opswall", "stats", "alerts"],
...
export default function App() {
  ...
  return (
    <div className="mg-root">
      ...
      />
      <div style={{ padding: 14, maxWidth: tab === "opswall" ? 1100 : 720, margin: "0 auto" }}>
