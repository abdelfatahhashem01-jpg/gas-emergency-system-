...
function defaultAppSettings() {
  return { appName: "نظام طوارئ الغاز", roleIcons: {}, severityIcons: {}, whatsappNumbers: [], zoomLink: "" };
}
...
function applyAppSettings(s) {
  dynamicSettings = {
    ...
    whatsappNumbers: (s && s.whatsappNumbers) || [],
...
function WhatsAppSendModal({ report, regions, onClose }) {
  const numbers = dynamicSettings.whatsappNumbers || [];
  const [selected, setSelected] = useState(() => Object.fromEntries(numbers.map((n) => [n.phone, true])));
  const [sentTo, setSentTo] = useState({});
  const message = buildWhatsAppAlertMessage(report, regions);

  function toggle(phone) { setSelected((s) => ({ ...s, [phone]: !s[phone] })); }
  function selectAll(v) { setSelected(Object.fromEntries(numbers.map((n) => [n.phone, v]))); }
  function sendTo(n) {
    const digits = (n.phone || "").replace(/[^\d]/g, "");
    if (!digits) return;
    window.open(`https://wa.me/${digits}?text=${encodeURIComponent(message)}`, "_blank");
    setSentTo((s) => ({ ...s, [n.phone]: true }));
  }
      ...
      {numbers.length === 0 ? (
        ...
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
            {numbers.map((n) => (
              <div key={n.phone} style={{ display: "flex", alignItems: "center", gap: 10, background: "#FBF0EF", borderRadius: 8, padding: "8px 10px" }}>
                <input type="checkbox" checked={!!selected[n.phone]} onChange={() => toggle(n.phone)} />
                <div style={{ flex: 1 }}>
                ...
                <button
                  type="button" className="mg-btn mg-btn-sm"
                  style={{ background: sentTo[n.phone] ? "#188038" : "#25D366", color: "#fff" }}
                  disabled={!selected[n.phone]}
                  onClick={() => sendTo(n)}
                >
                  {sentTo[n.phone] ? "✅ أُرسل" : "📤 إرسال"}
                </button>
              </div>
            ))}
          </div>
...
function SettingsPanel({ settings, onSaved }) {
  ...
  function addWhatsappNumber() {
    setForm((f) => ({ ...f, whatsappNumbers: [...f.whatsappNumbers, { name: "", phone: "" }] }));
  }
  function setWhatsappNumber(i, key, val) {
    ...
    <div className="mg-card">
      ...
      <div style={{ marginTop: 20, paddingTop: 14, borderTop: "1px dashed #EBD9D6" }}>
        ...
        <p style={{ fontSize: 12.5, color: "#8A6C68", marginTop: 0, marginBottom: 10 }}>
          السوبر أدمن ومجموعة المهندسين — تُرسل لهم رسالة واتساب فورية عند تسجيل بلاغ
          شديد الخطورة (انفجار خط غاز 7 بار، كسر خط غاز، حريق، حريق منظم، حادثة محطة غاز).
        </p>
        {form.whatsappNumbers.map((n, i) => (
          <div key={i} className="mg-grid2" style={{ marginBottom: 8, alignItems: "center" }}>
            <input className="mg-input" placeholder="الاسم (مثال: م. أحمد)" value={n.name} onChange={(e) => setWhatsappNumber(i, "name", e.target.value)} />
            <div style={{ display: "flex", gap: 6 }}>
              <input className="mg-input" placeholder="رقم واتساب مع كود الدولة (2010xxxxxxxx)" value={n.phone} onChange={(e) => setWhatsappNumber(i, "phone", e.target.value)} />
              <button type="button" className="mg-btn mg-btn-outline mg-btn-sm" onClick={() => removeWhatsappNumber(i)}>🗑️</button>
            </div>
          </div>
        ))}
        <button type="button" className="mg-btn mg-btn-ghost mg-btn-sm" style={{ width: "100%", justifyContent: "center" }} onClick={addWhatsappNumber}>
          ➕ إضافة رقم
        </button>
      </div>
