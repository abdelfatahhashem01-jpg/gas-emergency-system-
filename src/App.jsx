...
function defaultAppSettings() {
  return { appName: "نظام طوارئ الغاز", roleIcons: {}, severityIcons: {}, whatsappNumbers: [] };
}
...
// يبني رسالة التنبيه ويحاول إرسالها عبر واتساب لكل الأرقام المسجّلة
// (السوبر أدمن + مجموعة المهندسين) عند وقوع حادثة شديدة الخطورة.
// - لو سيرفر الشركة (SELF_API_BASE) مزوَّد بنقطة /api/whatsapp متصلة بـ
//   Meta WhatsApp Business API، يتم الإرسال تلقائياً 100% من غير أي تدخل.
// - غير كده (الوضع الافتراضي)، يفتح رسالة واتساب جاهزة الصياغة لكل رقم،
//   ويكفي ضغطة "إرسال" واحدة من الموظف لإتمام التنبيه.
async function sendWhatsAppAlerts(report) {
  ...
  if (SELF_API_BASE) {
    try {
      await fetch(`${SELF_API_BASE}/api/whatsapp`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(SELF_API_KEY ? { "x-api-key": SELF_API_KEY } : {}) },
        body: JSON.stringify({ numbers: numbers.map((n) => n.phone), message }),
      });
      return;
    } catch (e) {
      // فشل الإرسال التلقائي عبر السيرفر — ننتقل لفتح روابط واتساب يدوياً كخطة بديلة
    }
  }
  numbers.forEach((n, i) => {
