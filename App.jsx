...
function defaultAppSettings() {
  return { appName: "نظام طوارئ الغاز", roleIcons: {}, severityIcons: {} };
}
...
async function loadJSON(key, fallback) {
  // أولاً: نحاول أولاً من Firestore — المصدر المشترك بين كل المستخدمين والأجهزة
  try {
  ...
  // ثانياً: نسخة محلية احتياطية (localStorage / ذاكرة الجلسة) لو مفيش اتصال بالإنترنت
  try {
    if (hasLocalStorage()) {
...
// يعيد { persisted, error }. يكتب في Firestore (المصدر المشترك) وفي localStorage
// كنسخة احتياطية محلية في نفس الوقت. لو تعذّر الاتصال بالإنترنت، يحفظ محلياً فقط
// ولا يمنع التطبيق من الاستمرار — البيانات تُزامَن لاحقاً في أول اتصال ناجح.
...
function SettingsPanel({ settings, onSaved }) {
  ...
  return (
    <div className="mg-card">
      ...
      <div style={{ marginTop: 22, paddingTop: 14, borderTop: "1px dashed #EBD9D6", fontSize: 12.5, color: "#8A6C68" }}>
